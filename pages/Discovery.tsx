...
const Discovery: React.FC<DiscoveryProps> = ({ onBookClick, onNotificationClick, hasUnread }) => {
  ...
  // 模拟楼层坐标数据
  const mapPoints = books.slice(0, 3).map((book, index) => {
    const points = [
      { id: '1', top: '25%', left: '30%', label: '4F 产品部' },
      { id: '2', top: '65%', left: '70%', label: '2F 人力资源部' },
      { id: '3', top: '40%', left: '60%', label: '5F 技术部' }
    ];
    return { ...points[index], book };
  }).filter(p => p.book); // Ensure we have books for points
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3">
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg">
          <button
            ...
            className={`px-3 py-1 rounded-md text-[10px] font-bold shadow-sm flex items-center gap-1 transition-all ${viewMode === 'list' ? 'bg-white text-text' : 'text-text-muted'
              }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1 rounded-md text-[10px] font-bold flex items-center gap-1 transition-all ${viewMode === 'map' ? 'bg-white text-text shadow-sm' : 'text-text-muted'
              }`}
          >
